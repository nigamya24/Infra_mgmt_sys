...
class Tickets extends Database {  
	...
	public function showTickets(){
		...
		$time = new time;  			 
		$sqlQuery = "SELECT t.id, t.uniqid, t.title, t.init_msg as message, t.date, t.last_reply, t.resolved, t.priority as priority, u.name as creater, d.name as department, u.user_type, t.user, t.user_read, t.admin_read, t.customer as customer
			FROM tickets t 
	...
	public function createTicket() {      
		...
		if(!empty($_POST['subject']) && !empty($_POST['message'])) {                
			...
			$queryInsert = "INSERT INTO ".$this->ticketTable." (uniqid, user, customer, title, init_msg, department, date, last_reply, user_read, admin_read, resolved, priority) 
			VALUES('".$uniqid."', '".$_SESSION["userid"]."', '".$_POST['customer']."', '".$_POST['subject']."', '".$_POST['message']."', '".$_POST['department']."', '".$date."', '".$_SESSION["userid"]."', 0, 0, '".$_POST['status']."', '$priority')";			
	...
	public function updateTicket() {
		if($_POST['ticketId']) {	
			...
			SET title = '".$_POST["subject"]."', department = '".$_POST["department"]."', init_msg = '".$_POST["message"]."', resolved = '".$_POST["status"]."', priority = '".$_POST["priority"]."'
			WHERE id ='".$_POST["ticketId"]."'";
